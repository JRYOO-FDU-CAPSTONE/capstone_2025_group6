...
class QueueCache(EvictionPolicy):
    def __init__(self, evictions_log, num_elems, ap, *,
        ...
        if options.eviction_policy == 'ttl-ml':
            ...
            from BCacheSim.cachesim.eviction_policies import DT_SLRU_EvictionPolicy
            self.cache = DT_SLRU_EvictionPolicy(options)

        elif options.eviction_policy == 'EDE':
            # Use the same TTL-based queue, but predict TTL via EpisodeDeadlineEvictionPolicy
            self.cache = TTLPolicy()                      # ensure TTL queue is used
            self.ttl_predicter = EpisodeDeadlineEvictionPolicy(options)
    ...
    def admit(self, key, ts, *, ttl=None, ts_access=None, episode=None, **item_kwargs):
        ...
        # --- begin DT-per-byte hook for E1 ---
        md = item_kwargs.get("metadata") if "metadata" in item_kwargs else None
        if md:
            # Prefer explicit dt_per_byte if your pipeline provides it
            dtpb = md.get("dt_per_byte")
            if dtpb is None:
                # Fallback: compute a reasonable proxy if available
                size_b = md.get("size_bytes") or md.get("size") or 0
                st_nc  = md.get("service_time_nocache") or md.get("service_time_no_cache") or 0.0
                if size_b:
                    dtpb = st_nc / float(size_b)
            if dtpb is not None:
                # Attach to the admitted item's stats so DT-SLRU can read it in touch()
                self.cache[key].stats["dt_per_byte"] = dtpb
        # --- end DT-per-byte hook ---
...
class EpisodeDeadlineEvictionPolicy(EvictionImpl):
    """
    EDE (Episode-Deadline Eviction) with the simple four-hook interface.
    - Two segments tracked by LRU: Probation (bait=True) and Protected (bait=False)
    - PROTECTED cap: fraction of cache bytes reserved for Protected
    - α_tti: EWMA smoothing for time-to-idle (drives per-key TTL/expiry estimate)
    """

    def __init__(self, options):
        # required by EvictionImpl
        self.items = {}                 # key -> TTLItem (or QueueItemWithStats)
        # segments (LRU)
        self._prob = OrderedDict()      # key -> None
        self._prot = OrderedDict()      # key -> None

        # EDE knobs
        self.alpha = getattr(options, "ede_alpha_tti", 0.70)     # EWMA for TTI
        self.cap_frac = getattr(options, "ede_protected_cap", 0.25)

        # state
        self._last_ts = {}              # key -> last access ts
        self._ewma_tti = {}             # key -> EWMA TTI
        self._default_tti = 60.0
        self._default_ttl = 60.0     # <-- add this line (used by predict())

        # byte accounting to enforce cap as a fraction of current occupancy
        self._bytes_total = 0
        self._bytes_prot = 0

    # --- helper: coerce many ts types to POSIX seconds (float) ---
    def _to_secs(self, ts):
        # already numeric
        if isinstance(ts, (int, float)):
            return float(ts)

        # has .timestamp() (datetime-like)
        ...
        try:
            ...
            if isinstance(ts, np.datetime64):
                ...
                return float(ts.astype("datetime64[s]").astype("int64"))
        except Exception:
            pass

        # common alternates some pipelines use
        if isinstance(ts, dict):
            for k in ("ts_secs", "ts_sec", "ts_float"):
    ...
    def predict(self, features, metadata=None):
        md = metadata or {}
        key = md.get("key")
        ts_obj = md.get("ts", 0.0)
        now = self._to_secs(ts_obj)      # your robust ts->float helper

        if not key or now <= 0.0:
            return self._default_ttl     # <-- use _default_ttl

        last = self._last_ts.get(key)
        if last is not None:
            gap = max(0.0, now - last)
            prev = self._ewma_tti.get(key, gap)
            self._ewma_tti[key] = self.alpha * gap + (1.0 - self.alpha) * prev

        self._last_ts[key] = now
        return max(1.0, self._ewma_tti.get(key, self._default_ttl))  # <-- use _default_ttl


    # ------------ four-hook interface ------------
    def on_admit(self, key, item, metadata, now):
        """New item enters cache. Compute initial TTI and choose segment."""
        # EWMA bootstrap
        tti = self._default_tti
        self._ewma_tti[key] = tti
        self._last_ts[key] = now

        # size for accounting
        sz = getattr(item, "size", None) or (metadata.get("size_bytes") if metadata else 0) or 0
        self._bytes_total += sz

        # value signal (prefer dt_per_byte if present; fallback to service_time/size)
        dtpb = None
        if metadata:
            dtpb = metadata.get("dt_per_byte")
            if dtpb is None:
                st_nc = metadata.get("service_time_nocache") or metadata.get("service_time_no_cache") or 0.0
                szm   = metadata.get("size_bytes") or metadata.get("size") or sz
                if szm:
                    dtpb = st_nc / float(szm)

        # Decide protection under cap (greedy): protect only if valuable and under cap
        cap_bytes = self.cap_frac * max(1, self._bytes_total)
        want_protect = (dtpb is not None and dtpb > 0)

        if want_protect and (self._bytes_prot + sz) <= cap_bytes:
            # mark non-bait (protected) and LRU-track in Protected
            if hasattr(item, "setBait"):
                item.setBait(False)
            elif hasattr(item, "is_bait"):
                item.is_bait = False
            self._prot[key] = None
            self._bytes_prot += sz
        else:
            # keep bait=True (probation)
            if hasattr(item, "setBait"):
                item.setBait(True)
            elif hasattr(item, "is_bait"):
                item.is_bait = True
    ...
    def on_hit(self, key, item, metadata, now):
        """Update EWMA TTI, refresh LRU position, and (optionally) reclassify."""
        # EWMA update
        last = self._last_ts.get(key)
        if last is not None:
            gap = max(0.0, now - last)
        ...
        # move to MRU within its segment
        if key in self._prob:
            self._prob.move_to_end(key)
        elif key in self._prot:
            self._prot.move_to_end(key)

        # (Optional) opportunistic re-protection if room opens up
        if key in self._prob:
            sz = getattr(item, "size", 0)
            cap_bytes = self.cap_frac * max(1, self._bytes_total)
            if (self._bytes_prot + sz) <= cap_bytes:
    ...
    def pick_victim(self):
        """Choose a victim key: prefer LRU from Probation, else from Protected."""
        if self._prob:
            # Probation LRU (bait first)
            k = next(iter(self._prob))
            return k
        if self._prot:
            # fallback: Protected LRU
            k = next(iter(self._prot))
            return k
        return None

    def on_evict(self, key, item):
        """Cleanup and accounting when victim leaves."""
        # remove from segments
        self._prob.pop(key, None)
        self._prot.pop(key, None)
        # byte accounting
        sz = getattr(item, "size", 0)
        self._bytes_total = max(0, self._bytes_total - sz)
        # was protected?
        was_prot = False
        if hasattr(item, "isBait"):
            was_prot = not item.isBait()
        elif hasattr(item, "is_bait"):
            was_prot = not bool(item.is_bait)
        if was_prot:
            self._bytes_prot = max(0, self._bytes_prot - sz)
        # drop predictors
        self._last_ts.pop(key, None)
        self._ewma_tti.pop(key, None)

    # ------------ adapters to artifact's EvictionImpl API ------------
    def admit(self, key, item):
        """Wrapper called by QueueCache."""
        self.items[key] = item
        md = getattr(item, "stats", None) or {}
        now = getattr(item, "last_access_time", 0.0)
        self.on_admit(key, item, md, now)

    def touch(self, key):
        item = self.items[key]
        md = getattr(item, "stats", None) or {}
        now = getattr(item, "last_access_time", 0.0)
        self.on_hit(key, item, md, now)
    ...
    def evict(self, key=None):
        if key is None:
            ...
            if item is None:
                return None
        # remove from EvictionImpl store
...
class DT_SLRU_EvictionPolicy(EvictionImpl):
    """
    E1: DT-SLRU (Segmented LRU with DT-aware promotion).
    Two segments: Probation (new/low-value) and Protected (promoted).
    Promotion on (a) second hit or (b) dt_per_byte >= tau.
    Evict from Probation first; if empty, from Protected.
    """
    ...
    def __init__(self, options):
        ...
        # Segments maintain LRU order of keys
        self.probation = OrderedDict()  # key -> None
        self.protected = OrderedDict()  # key -> None

        # Threshold τ_DT (promotion selectivity)
        self.tau_dt = getattr(options, "dt_slru_tau", 1.0)

    # ---- EvictionImpl required API ----

    def admit(self, key, item):
        # Insert new item to Probation
        self.items[key] = item
        self.probation[key] = None  # newest at end by default

    def touch(self, key):
        """Called on hit; maintain LRU order and handle promotions."""
        if key in self.probation:
            # LRU move first
            self.probation.move_to_end(key)
            it = self.items[key]
            # dt_per_byte is optional — promotion also happens on second hit
            dtpb = it.stats.get("dt_per_byte") if hasattr(it, "stats") else None
            if it.hits >= 2 or (dtpb is not None and dtpb >= self.tau_dt):
                # Promote to Protected
                self.probation.pop(key, None)
                self.protected[key] = None
        elif key in self.protected:
            self.protected.move_to_end(key)
        # else: key not present (shouldn't happen on touch)

    def evict(self, key=None):
        """
        Evict one item.
        - If key is provided: evict that specific key (wherever it is).
        - Else: evict from Probation LRU head, else from Protected LRU head.
        Returns (key, item) like LRUPolicy/TTLPolicy do.
        """
        if key is not None:
            if key in self.probation:
                self.probation.pop(key, None)
        ...
        # No key: choose victim from Probation first
        if self.probation:
    ...
    def victim(self):
        """Return a peek at the would-be evicted pair, or None."""
        if self.probation:
            k = next(iter(self.probation))
            return (k, self.items[k])
        if self.protected:
            k = next(iter(self.protected))
            return (k, self.items[k])
        return None
